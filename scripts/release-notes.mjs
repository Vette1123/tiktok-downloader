/**
 * Release notes for the deploy that just shipped, built from the commits in it.
 *
 *   node scripts/release-notes.mjs         print the notes
 *   node scripts/release-notes.mjs --tag   print just the tag it would use
 *
 * The site is continuously deployed and there is nothing to install, so a
 * release here is a dated public record of what a deploy changed, not a
 * package. That is why the tag is a date (`v2026.08.31`, `.1` on a second
 * release the same day) rather than semver: the repo is public, the API is
 * ours and unversioned, and a bumped minor would mean nothing to a reader.
 *
 * Notes come from Conventional Commit subjects, which this repo already
 * writes. `feat`/`fix`/`perf`/`revert` are what a reader came for; docs,
 * chores, tests, refactors and CI are real work that says nothing outside the
 * repo, so they are counted rather than listed. A push carrying nothing but
 * those publishes no release at all: the `has_visible` output written at the
 * bottom is what the publish step's `if:` reads in
...
/** Newest `v*` tag by creation date, or null on the first ever run. */
function lastTag() {
  try {
 ...
 *
 * UTC, so a deploy at 01:00 Cairo does not get yesterday's date on one machine
 * and today's on another.
 */
function nextTag() {
...
/** `fix(resolve): unwrap Meta's login wall` → `{ type, scope, summary }`. */
 ...
 * would be the entire history, publishing a "release" that lists two years of
