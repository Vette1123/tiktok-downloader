...
| 2026-08-31 | [Dated releases on deploy, ported from the sibling repo](2026-08-31-dated-releases-on-deploy.md) | Two of the three changes copied from the sibling repo had no call site here — the beacon fix targets a RUM step this repo never had, and the hydration fix targets fallback routes a static export does not have; the release job that stayed needs its own `contents: write` job so the deploy job never holds a token its build scripts could use. |
